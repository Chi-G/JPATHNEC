        ...
        $query = Order::where('user_id', $user->id)
            ...
            $orders->getCollection()->transform(function ($order) {
                return [
                    ...
                    'delivery_status_message' => $order->getDeliveryStatusMessage(),
                    'is_delivery_overdue' => $order->isDeliveryOverdue(),
    ...
    public function track($id)
    {
        ...
        return Inertia::render('my-orders/tracking', [
            'order' => [
                ...
                'payment_status' => $order->payment_status,
                'total_amount' => $order->total_amount,
                'formatted_total' => $order->currency . number_format((float)$order->total_amount, 2),
                'currency' => $order->currency ?? '₦',
                'created_at' => $order->created_at->toISOString(),
                'shipped_at' => $order->shipped_at?->toISOString(),
                'delivered_at' => $order->delivered_at?->toISOString(),
