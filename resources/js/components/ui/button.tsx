...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({
    ...
    const calculatedIconSize = iconSize || (size ? iconSizeMap[size] : undefined) || 16;
    ...
    if (asChild) {
        try {
            ...
            const childElement = child as React.ReactElement<React.HTMLAttributes<HTMLElement> & { disabled?: boolean }>;
            const content = (
                <>
                    {loading && <LoadingSpinner />}
                    {iconName && iconPosition === 'left' && renderIcon()}
            ...
            const clonedChild = React.cloneElement(childElement, {
                ...
                disabled: disabled || loading || childElement?.props?.disabled,
                children: content,
            });
